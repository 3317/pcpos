...
//            left += g.getFontMetrics().getStringBounds(t.text, g).getWidth();
            w = (float) g.getFontMetrics().getStringBounds("w", g).getWidth();
            for (p = 0; t.text.length() > p; p++) {
                g.drawString(t.text.substring(p,p+1), left, (float) y);
                if (checkFontWidth(t.text.charAt(p))) {
                    left += w;
                }
                left += w;
            }
