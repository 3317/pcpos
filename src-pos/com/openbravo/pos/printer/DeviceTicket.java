...
public static String getWhiteString2(int iSize) {
    ...
    public static String alignLeft(String sLine, int iSize) {
        ...
        int p;
        l=0;
        for(p=0;sLine.length()>p;p++){
            if(checkFontWidth(sLine.charAt(p))){
                l++;
            }
            l++;
            if(l>=iSize){
...
//        }
        if(l>=iSize){
            return sLine.substring(0,p);
        } else {
            return sLine +  getWhiteString(iSize - l);
        }
    }
...
//        }
        if(l>=iSize){
            return sLine.substring(p);
        } else {
            return getWhiteString(iSize - l) + sLine;
        }
}
...
//        }
        ...
        if(l>=iSize){
            return alignRight(sLine.substring(0, (p + iSize) / 2), iSize);
//            return sLine.substring(p);
        } else {
            return alignRight(sLine + getWhiteString((iSize - l) / 2), iSize);
//            return getWhiteString2(iSize - l) + sLine;
        }        
    }
    ...
    public static boolean checkFontWidth(char c){
